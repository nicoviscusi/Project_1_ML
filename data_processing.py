...
def handle_outliers(data):
    for i in range(data.shape[1]-5):
        ...
        data[data[:,i]>-999,i] = np.where(np.logical_or(data_test<lower_bridge,data_test>upper_bridge),median,data_test)
    return data
...
def handle_skew(data,Indices):
    for i in Indices:
        data_test = data[data[:,i]>-999][:,i]
        data[data[:,i]>-999][:,i] = np.log(data_test)
    return data
